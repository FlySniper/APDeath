...
async def ap_server(death_count, client):
    ...
    if os.name == "nt":
        ...
        if OPENSSL:
            p = pexpect.spawn(
                f"bash -c \"{ap_server_file} --host 0.0.0.0 --port {PORT} --hint_cost 10 --cert {CERT_NAME} --cert_key {CERT_KEY_NAME} {output_file}\"\n",
                timeout=None, encoding="utf-8", ignore_sighup=True)
        else:
